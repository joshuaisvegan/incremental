function test(){
  ...
  var countriesTemplate = document.createElement("template");
   input = document.getElementById("increment");
  for (var i = 0; i < list.length; i++){
    var names = document.createElement("option")
...
input.addEventListener("keyup", expand);
...
function expand(event){
  ...
  templateElementscopy.forEach(function(countryName){
    if (countryName.value.indexOf(input.value) === 0 && temp < 5){
      temp++;
