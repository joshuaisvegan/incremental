function test(){
  ...
  var countriesTemplate = document.createElement("template");
  var input = document.getElementById("increment");
  var templateElements = countriesTemplate.children;
  var names;
  for (var i = 0; i < list.length; i++){
    names = document.createElement("option")
    names.setAttribute("value", list[i]);
    countriesTemplate.appendChild(names);
  }
  ...
  function expand(event){
    ...
    var capitalize = inputValue[0].toUpperCase() + inputValue.slice(1,inputValue.length)
    countries.innerHTML = "";
    templateElementscopy = [].slice.call(templateElements)

    var caps = inputValue.split(" ")
    console.log(caps);
    for (var i = 0; i <caps.length; i++){
      if(caps[i] !== ''){
        caps[i]= caps[i][0].toUpperCase() + caps[i].slice(1,caps[i].length);
        console.log(caps)
      }

    }
    capitalise = caps.join(" ");

    templateElementscopy.forEach(function(countryName){
      console.log(capitalize)
      if (countryName.value.indexOf(capitalize) === 0 && temp < 5){
        temp++;
        noString = document.createElement("option");
        noString.value = countryName.value
        countries.appendChild(noString);
      }
    });
  }
}
